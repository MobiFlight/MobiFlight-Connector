...
{
    [TestClass]
    public class WinCtrlCduControllerTests
    {
        ...
        private WinCtrlCduController CreateDevice(WinCtrlCduType type)
        {
            return new WinCtrlCduController(mockMessageSender, type);
        }

        private WinCtrlCduController CreateMcdu() => CreateDevice(WinCtrlCduType.MCDU);
            ...
            int highByte)
        {
            ...
            var (actualLowByte, actualHighByte) = WinCtrlCduController.GetFormatBytes(token, out _);
        ...
        public void GetFormatBytes_InvertedFlag_AddsColorInvertOffset()
        {
            ...
            var (low, high) = WinCtrlCduController.GetFormatBytes(token, out var c);
        ...
        public void GetFormatBytes_NotInvertedFlag_DoesNotAddOffset()
        {
            ...
            var (low, high) = WinCtrlCduController.GetFormatBytes(token, out _);
        ...
        public void GetFormatBytes_ThreeElementToken_DefaultsToNotInverted()
        {
            ...
            var (low, _) = WinCtrlCduController.GetFormatBytes(token, out _);
        ...
        {
            var (l, h) = WinCtrlCduController.GetFormatBytes(token, out var ch);
            return (l, h, ch);
        }
        ...
        [TestMethod]
        [DataRow((int)WinCtrlCduType.MCDU,  "WinCtrl MCDU")]
        [DataRow((int)WinCtrlCduType.PFP3N, "WinCtrl PFP3N")]
        [DataRow((int)WinCtrlCduType.PFP4,  "WinCtrl PFP4")]
        [DataRow((int)WinCtrlCduType.PFP7,  "WinCtrl PFP7")]
        public void Name_PerCduType_ReturnsExpected(int typeAsInt, string expectedName)
        {
            var device = CreateDevice((WinCtrlCduType)typeAsInt);
        ...
        [TestMethod]
        [DataRow((int)WinCtrlCduType.PFP3N, "CALL")]
        [DataRow((int)WinCtrlCduType.PFP4,  "DSPY")]
        [DataRow((int)WinCtrlCduType.PFP7,  "DSPY")]
        ...
        {
            var device = CreateDevice(WinCtrlCduType.PFP3N);
