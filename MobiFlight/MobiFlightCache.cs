...
namespace MobiFlight
{
    ...
    {    
        ...
        {
            if (connectedComModules.Count == 0) return false;
...
*/
        public bool connectAsync(bool force=false)
        {
        ...
        internal void OnBoardDiscovered(MobiFlightModuleInfo devInfo)
        {
            connectedComModules.Add(devInfo);

            if (!devInfo.HasMfFirmware()) return;

            MobiFlightModule module= new MobiFlightModule(devInfo);
            RegisterModule(module, devInfo);
