...
| Config List | Reorder / Drag & Drop | Config Item(s) | Move | Backend + frontend synchronization | Project-persisted | Item GUID(s) + original config + original index/indices | Yes | ConfigItemTableActiveCell.tsx → DnD utilities → CommandResortConfigItem → ExecutionManager | Original position are already tracked during dragging |
