...
test('Go to configs', async ({ mfPage, page }) => {
  ...
  await expect(page.getByRole('cell', { name: 'My first item' })).toHaveCount(1)
...
test('Log messages are displayed correctly', async ({ mfPage, page }) => {
  ...
  const logMessage2:Types.ILogMessage = { Message: "This is a 2nd test message",  Severity: 'info', Timestamp: new Date()}
