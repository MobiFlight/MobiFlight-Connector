...
import {detectI18nUsage, shouldIgnoreContext, shouldIgnoreFile, shouldIgnoreLine,shouldIgnoreString,extractHardcodedStrings} from "./modules/i18n-detection-methods.js"
...
function generateReport(results) {
  ...
  const totalStrings = results.reduce(
    (sum, file) => sum + file.hardcodedStrings.length,
    0,
  )
...
function generateGitHubComment(results) {
  ...
  results.slice(0, 10).forEach((file) => {
    // Limit to first 10 files to avoid huge comments
    comment += `**${file.file}** (${file.hardcodedStrings.length} issues)\n`
    file.hardcodedStrings.slice(0, 5).forEach((issue) => {
  ...
  comment += "</details>\n\n"
  comment +=
    "💡 **Tip**: Use the `t()` function from `react-i18next` to make strings translatable."
