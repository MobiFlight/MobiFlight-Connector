        ...
        public static List<DeviceReference> Create(BaseDevice dev)
        {
            ...
            switch (dev.Type)
            {
                case DeviceType.Button:
                    result.Add(new DeviceReference { Name = dev.Name, Type = DeviceType.Button });
                    break;

                case DeviceType.Encoder:
                    result.Add(new DeviceReference { Name = dev.Name, Type = DeviceType.Encoder });
                    break;

                case DeviceType.AnalogInput:
                    result.Add(new DeviceReference { Name = dev.Name, Type = DeviceType.AnalogInput });
                    break;
                ...
                case DeviceType.InputMultiplexer:
                    ...
                    for (int i = 0; i < muxPinCount; i++)
                        result.Add(new DeviceReference { Name = $"{dev.Name}:{i}", Type = DeviceType.Button });
                    break;
