...
{
    internal class WinCtrlPap3 : WinCtrlBaseController
    {
        ...
        private WinCtrlPap3Report CurrentReport = new WinCtrlPap3Report();
        private WinCtrlPap3Report PreviousReport = new WinCtrlPap3Report();

        public WinCtrlPap3(SharpDX.DirectInput.Joystick joystick, JoystickDefinition def, int productId, WebSocketServer server) : base(joystick, def, productId, server)
        {
        ...
        public override void Connect(IntPtr handle)
        {
            ...
            if (Device == null)
            {
                ...
                if (Device == null)
                {
                    Log.Instance.log($"No WINCTRL PAP3 found with VID:{VendorId:X4} and PID:{ProductId:X4}", LogSeverity.Error);
                    return;
                }
            }
            ...
            if (Stream == null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    // abort the scan.
                    Log.Instance.log($"Failed to open WINCTRL PAP3: {ex.Message}", LogSeverity.Error);
                    return;
                }
            }
            ...
            {
                Receiver.Start(Stream, Device.GetMaxInputReportLength(), OnReportReceived, OnReadError, $"WinCtrlPap3-{ProductId:X4}");
            }
        ...
        {
            // Exception when disconnecting pap3 while mobiflight is running.
            Log.Instance.log($"WINCTRL PAP3 read failed, shutting down: {exception.Message}", LogSeverity.Error);
            Shutdown();
        }
        ...
        protected override void EnumerateDevices()
        {
            ...
            foreach (var input in Definition.Inputs)
            {
                ...
                Log.Instance.log($"Added WINCTRL PAP3 Id: {input.Id} Axis: {input.Name} Label: {input.Label}.", LogSeverity.Debug);
            }
        }
