...
{
    internal class EfisCube : Joystick
    {
        /// <summary>
        ...
        /// <summary>
        /// The report implementation for the EFIS Cube.
        /// </summary>
        private readonly EfisCubeReport EfisCubeReport = new EfisCubeReport();
        ...
        public EfisCube(JoystickDefinition definition) : base(null, definition)
        {
        }
        ...
        protected async Task<bool> Connect()
        {
            ...
            {
                Log.Instance.log($"no EfisCube found with VID:{VendorId.ToString("X4")} and PID:{ProductId.ToString("X4")}", LogSeverity.Info);
                return false;
            }
        ...
        protected void ProcessInputReportBuffer(byte reportId, byte[] inputReportBuffer) {
            var newState = EfisCubeReport.Parse(inputReportBuffer).ToJoystickState();
        ...
        protected async override void SendData(byte[] data)
        {
            ...
            catch (Exception ex)
            {
                ...
                Log.Instance.log($"Exception during write to EfisCube ({ex.GetType().Name}): {ex.Message}", LogSeverity.Error);
                OnDeviceRemoved();
            }
        }
        ...
        public override void UpdateOutputDeviceStates()
        {
            ...
            var data = EfisCubeReport?.FromOutputDeviceState(OutputState);
        ...
        public override void Stop()
        {
            ...
            var data = EfisCubeReport?.FromOutputDeviceState(Lights);
            SendData(data);
