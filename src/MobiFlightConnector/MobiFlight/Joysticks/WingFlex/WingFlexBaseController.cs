...
namespace MobiFlight.Joysticks.WingFlex
{
    internal class WingFlexBaseController : Joystick
    {
        ...
        /// </summary>
        private readonly Dap500Report FcuCubeReport = new Dap500Report();
        ...
        {
            get { return Definition?.InstanceName ?? "WingFlex Controller"; }
        }
        ...
        {
            get { return $"{Joystick.SerialPrefix}{Device?.ConnectedDeviceDefinition?.SerialNumber}" ?? "WING-FLEX-1234-ABCD-12345678"; }
        }
        ...
        public WingFlexBaseController(JoystickDefinition definition) : base(null, definition)
        {
        }
        ...
        protected async Task<bool> Connect()
        {
            ...
            {
                Log.Instance.log($"no WingFlex Controller found with VID:{VendorId.ToString("X4")} and PID:{ProductId.ToString("X4")}", LogSeverity.Info);
                return false;
            }
            ...
            {
                IsBackground = true,
                Name = $"WingFlex-HID-Reader-{Device?.ConnectedDeviceDefinition?.SerialNumber}"
            };
            readThread.Start();
