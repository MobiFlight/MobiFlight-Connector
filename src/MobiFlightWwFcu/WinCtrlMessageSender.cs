...
{
    internal class WinCtrlMessageSender : IWinCtrlMessageSender
    {
        ...
        public WinCtrlMessageSender(int productId)
        {
            ProductId = productId;
        }
        ...
        private void WriteStream(byte[] buffer, int offset, int count)
        {
            if (Stream == null)
            {
                throw new ApplicationException("WinCtrlDisplayControl cannot send data. Not connected to device. Stream is null.");
            }
            lock (StreamLock)
            {
