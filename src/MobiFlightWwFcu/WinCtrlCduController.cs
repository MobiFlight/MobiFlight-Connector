...
{
    internal class WinCtrlCduController : IWinCtrlController
   {
        public string Name { get => $"WinCtrl {CduType}"; }

        private IWinCtrlMessageSender MessageSender = null;

        private WinCtrlCduType CduType = WinCtrlCduType.MCDU;

        private byte[] DestinationAddress = WinCtrlConstants.DEST_MCDU;    
        ...
        private WinCtrlFontConverter FontConverter = new WinCtrlFontConverter();
        ...
        public WinCtrlCduController(IWinCtrlMessageSender sender, WinCtrlCduType cduType)
        {
            MessageSender = sender;
            CduType = cduType;

            if (CduType == WinCtrlCduType.MCDU)
            {
                ...
                DestinationAddress = WinCtrlConstants.DEST_MCDU;
            ...
            else if (CduType == WinCtrlCduType.PFP3N)
            {
                InitCommandSequence.AddRange(InitCommandHeaderPfp3n);                
                InitCommandSequence.AddRange(InitCommandData);
                DestinationAddress = WinCtrlConstants.DEST_PFP3N;
                LedIdentifiers = new Dictionary<string, byte>()
                {
                    { $"CALL",   0x03 },
                    { $"FAIL",   0x04 },
            ...
            else if (CduType == WinCtrlCduType.PFP7)
            {
                InitCommandSequence.AddRange(InitCommandHeaderPfp3n);
                InitCommandSequence.AddRange(InitCommandData);
                DestinationAddress = WinCtrlConstants.DEST_PFP7;
                LedIdentifiers = new Dictionary<string, byte>()
                {
                    { $"DSPY",   0x03 },
                    { $"FAIL",   0x04 },
            ...
            else if (CduType == WinCtrlCduType.PFP4)
            {
                InitCommandSequence.AddRange(InitCommandHeaderPfp3n);
                InitCommandSequence.AddRange(InitCommandData);
                DestinationAddress = WinCtrlConstants.DEST_PFP4;
                LedIdentifiers = new Dictionary<string, byte>()
                {
                    { $"DSPY",   0x03 },
                    { $"FAIL",   0x04 },
        ...
        {     
            ...
            foreach (var cmd in InitCommandSequence)
            {
                ...
                fullCommand.AddRange(WinCtrlConstants.DisplayCmdHeaders[cmd.Item1]);
        ...
        public void Connect()
        {
            ...
            ConvertAndSendCduData(WinCtrlConstants.InitialDisplayJson);
        }
        ...
        {
            return new List<string>(new string[] { WinCtrlConstants.FONT_DATA, WinCtrlConstants.CDU_DATA });
        }
        ...
        {         
            if (!string.IsNullOrWhiteSpace(value))
            {
                ...
                else if (name == WinCtrlConstants.CDU_DATA)
                {
                    ConvertAndSendCduData(value);
                }
                else if (name == WinCtrlConstants.FONT_DATA)
                {
