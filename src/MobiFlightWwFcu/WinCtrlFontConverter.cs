...
{
    internal class WinCtrlFontConverter
    {
        ...
        private List<byte[]> CreateFontHeadCommand(WinCtrlFontHeadConfig fontHeadConfig, byte[] destinationAddress)
        {
            List<byte[]> commands = new List<byte[]>();
            List<byte> command = new List<byte>(destinationAddress);
            command.AddRange(new byte[2]);
            command.AddRange(WinCtrlConstants.DisplayCmdHeaders["0601"]);
        ...
        private List<byte[]> CreateFontCommands(WinCtrlFontConfig fontConfig, byte[] destinationAddress)
        {
            int offset = 0;
            List<byte[]> commands = new List<byte[]>();            
            ...
            foreach (var pixelData in pixelDataList)
            {
                ...
                List<byte> command = new List<byte>(destinationAddress);
                command.AddRange(new byte[2]);
                command.AddRange(WinCtrlConstants.DisplayCmdHeaders["0701"]);         
        ...
        public WinCtrlFontCommands FontJsonToDisplayCommands(string fontJson, byte[] destinationAddress)
        {           
            ...
            WinCtrlFontHeadConfig headConfigLarge = new WinCtrlFontHeadConfig();
            ...
            WinCtrlFontHeadConfig headConfigSmall = new WinCtrlFontHeadConfig();
            ...
            WinCtrlFontConfig largeFontConfig = new WinCtrlFontConfig();
            ...
            WinCtrlFontConfig smallFontConfig = new WinCtrlFontConfig();
            ...
            WinCtrlFontData fontData = new WinCtrlFontData();
            fontData.smallFontConfig = smallFontConfig;
            fontData.largeFontConfig = largeFontConfig;

            WinCtrlFontCommands fontCommands = new WinCtrlFontCommands();
