...
copy %TEMP_DIR%\mobiflight_micro.ino.hex %RELEASE_DIR%\mobiflight_micro_%VERSION%.hex

%ARDUINO_DIR%\arduino-builder -dump-prefs -logger=machine -hardware %ARDUINO_DIR%\hardware -hardware %FW_SOURCE_DIR%\packages -tools %ARDUINO_DIR%\tools-builder -tools %ARDUINO_DIR%\hardware\tools\avr -tools %FW_SOURCE_DIR%\packages -built-in-libraries %ARDUINO_DIR%\libraries -libraries %ARDUINO_DIR%-1.8.0\sketchbooks\libraries -fqbn=arduino:avr:uno -vid-pid=0X2341_0X0042 -ide-version=10800 -build-path %TEMP_DIR% -warnings=default -prefs=build.warn_data_percentage=75 -prefs=runtime.tools.avrdude.path=%ARDUINO_DIR%\hardware\tools\avr -prefs=runtime.tools.avr-gcc.path=%ARDUINO_DIR%\hardware\tools\avr -prefs=runtime.tools.arduinoOTA.path=%ARDUINO_DIR%\hardware\tools\avr -verbose %FW_SOURCE_DIR%\mobiflight_uno\mobiflight_uno.ino
%ARDUINO_DIR%\arduino-builder -compile -logger=machine -hardware %ARDUINO_DIR%\hardware -hardware %FW_SOURCE_DIR%\packages -tools %ARDUINO_DIR%\tools-builder -tools %ARDUINO_DIR%\hardware\tools\avr -tools %FW_SOURCE_DIR%\packages -built-in-libraries %ARDUINO_DIR%\libraries -libraries %ARDUINO_DIR%-1.8.0\sketchbooks\libraries -fqbn=arduino:avr:uno -vid-pid=0X2341_0X0042 -ide-version=10800 -build-path %TEMP_DIR% -warnings=default -prefs=build.warn_data_percentage=75 -prefs=runtime.tools.avrdude.path=%ARDUINO_DIR%\hardware\tools\avr -prefs=runtime.tools.avr-gcc.path=%ARDUINO_DIR%\hardware\tools\avr -prefs=runtime.tools.arduinoOTA.path=%ARDUINO_DIR%\hardware\tools\avr -verbose %FW_SOURCE_DIR%\mobiflight_uno\mobiflight_uno.ino
copy %TEMP_DIR%\mobiflight_uno.ino.hex %RELEASE_DIR%\mobiflight_uno_%VERSION%.hex
